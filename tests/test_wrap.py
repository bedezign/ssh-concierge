            ...
            json.dumps(
                {
                    "myhost": {
                        "fields": {
                            "password": {
                                "original": "op://vault/item/password",
                                "resolved": None,
                                "sensitive": True,
                            },
                        },
...
class TestResolveFields:
    ...
    def test_sensitive_resolved_at_ssh_time(self, mock_op_cls):
        ...
        mock_op.resolve.return_value = "secret123"
        entry = {
            "fields": {
                "password": {
                    "original": "ops://V/I/password",
                    "resolved": None,
                    "sensitive": True,
                },
            },
    ...
    def test_otp_resolved_at_ssh_time(self, mock_op_cls):
        mock_op = mock_op_cls.return_value
        mock_op.resolve.side_effect = lambda fv: {
            "ops://V/I/password": "secret",
            "ops://V/I/one-time password": "123456",
        }.get(fv.original)
        entry = {
            "fields": {
                "password": {
                ...
                "otp": {
                    "original": "ops://V/I/one-time password",
                    "resolved": None,
                    "sensitive": True,
                },
            },
    ...
    def test_otp_failure_non_critical_in_resolve(self, mock_op_cls):
        ...
        mock_op = mock_op_cls.return_value
        mock_op.resolve.side_effect = (
            lambda fv: "secret" if fv.original == "ops://V/I/password" else None
        )
        entry = {
            "fields": {
                "password": {
    ...
    def test_fallback_chain(self, mock_op_cls):
        """The chain walk lives inside op.resolve now; wrap.py just delegates."""
        mock_op = mock_op_cls.return_value
        mock_op.resolve.return_value = "backup-pw"
        entry = {
            "fields": {
                "password": {
...
class TestCopyToClipboard:
    ...
    @patch(
        "ssh_concierge.wrap.subprocess.run", side_effect=FileNotFoundError("wl-copy")
    )
...
class TestMain:
    ...
    def test_no_hostdata_falls_through(self, mock_find, mock_execv, tmp_path: Path):
        with patch("sys.argv", ["/home/user/.local/bin/ssh", "myhost"]):
            with patch(
                "ssh_concierge.wrap.load_settings",
    ...
    @patch(
        "ssh_concierge.wrap.create_askpass",
        return_value={"SSH_ASKPASS": "/tmp/ap", "SSH_ASKPASS_REQUIRE": "force"},
    )
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_op_read_fails_falls_through(
        self, mock_find, mock_op_cls, mock_execv, tmp_path: Path
    ):
        mock_op = mock_op_cls.return_value
        mock_op.resolve.return_value = None
        hd_file = tmp_path / "hostdata.json"
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_password_and_clipboard(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
        ...
        mock_op = mock_op_cls.return_value
        mock_op.resolve.return_value = "secret"
        hd_file = tmp_path / "hostdata.json"
        hd_file.write_text(
            json.dumps(
                {
...
class TestMainNewFormat:
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_sensitive_password_resolved_at_ssh_time(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_clipboard_with_sensitive_field(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.os.execv")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_cached_non_sensitive_no_op_read(
        self, mock_find, mock_execv, tmp_path: Path
    ):
...
class TestMainPromptOverrides:
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_per_host_password_prompt_passed(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_per_host_otp_prompt_passed(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_otp_value_passed_to_askpass(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_otp_failure_non_critical(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
        ...
        mock_op = mock_op_cls.return_value
        mock_op.resolve.side_effect = (
            lambda fv: "secret" if fv.original == "ops://v/i/pw" else None
        )
        hd_file = tmp_path / "hostdata.json"
        hd_file.write_text(
            json.dumps(
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_no_prompt_overrides_passes_none(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
    ...
    @patch("ssh_concierge.wrap.OnePassword")
    @patch("ssh_concierge.wrap.find_real_binary", return_value="/usr/bin/ssh")
    def test_global_patterns_passed_from_settings(
        self,
        mock_find,
        mock_op_cls,
        mock_askpass,
        ...
        mock_askpass.assert_called_once()
        assert mock_askpass.call_args[1]["password_patterns"] == (
            "*assword*",
            "*ASSWORD*",
        )
