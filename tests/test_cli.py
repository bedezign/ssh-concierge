...
def _fake_summary(
    item_id: str, vault_id: str = "v1", category: str = "SSH_KEY"
) -> ItemSummary:
    """Build a minimal ItemSummary for mock responses."""
    return ItemSummary(
        id=item_id,
        title=item_id,
        vault_id=vault_id,
...
def _fake_item(item_id: str, vault_id: str = "v1", category: str = "SSH_KEY") -> Item:
    """Build a minimal Item for mock responses (no fields/sections)."""
    return Item(
        id=item_id,
        title=item_id,
        vault_id=vault_id,
...
class TestCmdGenerate:
    ...
    def test_generates_config(self, mock_op_cls, runtime_dir: Path):
        _configure_mock_op(mock_op_cls, ["id1", "id2"])
    ...
    def test_skips_unparseable_items(self, mock_op_cls, runtime_dir: Path):
        _configure_mock_op(mock_op_cls, ["id1"])
    ...
    @patch("ssh_concierge.cli.OnePassword")
    def test_host_filter_includes_matching(
        self, mock_op_cls, mock_socket, runtime_dir: Path
    ):
...
class TestCmdStatus:
    ...
    def test_ttl_zero_shows_manual(self, runtime_dir: Path, capsys):
        runtime_dir.mkdir(parents=True)
        conf = runtime_dir / "hosts.conf"
        conf.write_text(
            "# Generated by ssh-concierge\n\nHost prod\n    HostName 10.0.0.1\n"
        ...
        settings = dataclasses.replace(_make_settings(runtime_dir), ttl=0)
        cmd_status(settings)
        output = capsys.readouterr().out
...
SAMPLE_HOSTDATA = {
    "prod": {
        "fields": {
            "password": {
                "original": "op://Work/ServerLogin/password",
                "resolved": None,
                "sensitive": True,
            },
        },
...
def _key_item(
    ...
    title: str,
    vault_name: str,
    public_key: str | None = "ssh-ed25519 AAAA",
    fingerprint: str | None = "SHA256:abc",
    sectioned: bool = False,
) -> Item:
    """Build a minimal SSH_KEY Item carrying top-level key fields."""
    section_id = "sec" if sectioned else None
    ...
    sections = (ItemSection(id="sec", label="SSH Config"),) if sectioned else ()
    return Item(
        id=item_id,
        title=title,
        vault_id="v1",
...
class TestBuildKeyRegistry:
    def test_builds_from_items(self):
        items = [_key_item("key1", "My SSH Key", "Work")]
    ...
    def test_skips_items_without_key_data(self):
        items = [
            _key_item("nokey", "Server", "Work", public_key=None, fingerprint=None)
        ]
        registry = _build_key_registry(items)
    ...
    def test_skips_section_fields(self):
        ...
        items = [_key_item("key1", "Key", "Work", sectioned=True)]
        registry = _build_key_registry(items)
        assert len(registry) == 0
...
class TestResolveKeyRef:
    ...
    def test_url_encoded_slashes_in_item_name(self):
        registry = {
            ("work", "laptop / sn-001234 / john.doe"): (
                "ssh-ed25519 BBBB",
                "SHA256:xyz",
            ),
        }
    ...
    def test_self_ref_resolved_via_seeded_cache(self):
        ...
        op.read.assert_called_once_with("op://v1/i1/SSH Config/key")
    ...
    def test_explicit_field_ref_resolved_via_cache(self):
        ...
        op.read.assert_called_once_with("op://Work/MyItem/SSH Config/key")
    ...
    def test_field_resolution_status(self, runtime_dir: Path, capsys):
        ...
        hd = {
            "myhost": {
                "fields": {
                    "hostname": {
                        "original": "op://v1/i1/website",
                        "resolved": "new.example.com",
                        "sensitive": False,
                    },
                    "password": {
                        "original": "ops://v1/i1/SSH Config/password",
                        "resolved": None,
                        "sensitive": True,
                    },
                },
...
class TestResolveHostFieldsStaleness:
    ...
    def test_stale_reference_detected_via_cache_lookup(self):
        ...
        op = MagicMock()
        op.resolve.return_value = "new.example.com"
        ...
        host = HostConfig(
            aliases=["myhost"],
            hostname=FieldValue(
                original=ref, resolved=None, sensitive=False, field_type="reference"
            ),
        )
    ...
    def test_unchanged_reference_uses_cache(self):
        ...
        op = MagicMock()
        op.resolve.return_value = "same.example.com"
    ...
    def test_unknown_reference_falls_back_to_cache(self):
        """When resolve returns None (not in memory), keep the cached value."""
        op = MagicMock()
        op.resolve.return_value = None
        ...
        cached_fields = {
            "hostname": FieldValue(
                original=ref,
                resolved="cached.example.com",
                sensitive=False,
                field_type="reference",
            ),
        }
        result = resolve_host_fields(host, meta, cached_fields, op)
        # Should use cached value since resolve returned None
...
class TestWarnNoexecAskpass:
    ...
    def test_warns_on_noexec(self, tmp_path: Path, capsys):
        ...
        with patch("ssh_concierge.cli.os.statvfs", return_value=mock_result):
            with patch.object(
                os, "ST_NOEXEC", getattr(os, "ST_NOEXEC", 8), create=True
            ):
                _warn_noexec_askpass(tmp_path)
        err = capsys.readouterr().err
...
class TestWriteEnvSh:
    ...
    def test_sourceable_by_shell(self, settings, runtime_dir):
        ...
        result = subprocess.run(
            [
                "sh",
                "-c",
                f'. "{runtime_dir}/env.sh" && echo "CONFIG=$CONFIG" && echo "TTL=$TTL"',
            ],
            capture_output=True,
            text=True,
        )
    ...
    def test_writes_env_to_config_dir_only(self, tmp_path):
        """env.sh goes next to the config file when one exists, not in runtime_dir."""
        ...
        # Only config dir has env.sh
        assert (config_dir / "env.sh").exists()
...
class TestGetAgentFingerprints:
    ...
    def test_ssh_add_failure_raises(self):
        ...
        mock_result = MagicMock(
            returncode=1, stdout="", stderr="The agent has no identities.\n"
        )
        with patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/agent.sock"}):
            with patch("ssh_concierge.cli.subprocess.run", return_value=mock_result):
    ...
    def test_ssh_add_not_found_raises(self):
        ...
        with patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/agent.sock"}):
            with patch(
                "ssh_concierge.cli.subprocess.run", side_effect=FileNotFoundError
            ):
    ...
    def test_timeout_raises(self):
        ...
        with patch.dict(os.environ, {"SSH_AUTH_SOCK": "/tmp/agent.sock"}):
            with patch(
                "ssh_concierge.cli.subprocess.run",
                side_effect=sp.TimeoutExpired("ssh-add", 5),
            ):
...
class TestWarnMissingAgentKeys:
    ...
    def test_no_agent_warns(self, capsys):
        ...
        with patch(
            "ssh_concierge.cli._get_agent_fingerprints",
            side_effect=AgentQueryError("SSH_AUTH_SOCK not set"),
        ):
    ...
    def test_all_keys_present_no_warning(self, capsys):
        ...
        hosts = [HostConfig(aliases=["server1"], fingerprint="SHA256:abc")]
        with patch(
            "ssh_concierge.cli._get_agent_fingerprints", return_value={"SHA256:abc"}
        ):
            _warn_missing_agent_keys(hosts, {})
    ...
    def test_missing_key_warns(self, capsys):
        ...
        with patch(
            "ssh_concierge.cli._get_agent_fingerprints", return_value={"SHA256:other"}
        ):
    ...
    def test_wildcard_aliases_excluded(self, capsys):
        ...
        hosts = [
            HostConfig(aliases=["*.example.com", "server1"], fingerprint="SHA256:miss")
        ]
        with patch("ssh_concierge.cli._get_agent_fingerprints", return_value=set()):
            _warn_missing_agent_keys(hosts, {})
        err = capsys.readouterr().err
...
class TestAgentKeyStatus:
    ...
    def test_fingerprint_with_slashes(self):
        ...
        with patch("ssh_concierge.cli._get_agent_fingerprints", return_value={fp}):
            result = _agent_key_status(
                "    IdentityFile /run/keys/SHA256:abc_def_ghi.pub"
            )
        assert "✓" in result
